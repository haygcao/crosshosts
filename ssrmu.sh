...
Libsodiumr_file="/home/lib/libsodium.so"
...
Download_SSR(){
	...
	unzip "manyuser.zip"
	[[ ! -e "/home/shadowsocksr-manyuser/" ]] && echo -e "${Error} ShadowsocksR服务端 解压失败 !" && rm -rf manyuser.zip && exit 1
	mv "/home/shadowsocksr-manyuser/" "/home/shadowsocksr/"
	[[ ! -e "/home/shadowsocksr/" ]] && echo -e "${Error} ShadowsocksR服务端 重命名失败 !" && rm -rf manyuser.zip && rm -rf "/home/shadowsocksr-manyuser/" && exit 1
	rm -rf manyuser.zip
	cd "shadowsocksr"
...
Install_Libsodium(){
	...
	if [[ ${release} == "centos" ]]; then
		...
		echo /home/lib > /etc/ld.so.conf.d/usr_local_lib.conf
	else
		apt-get update
		echo -e "${Info} 安装依赖..."
...
Other_functions(){
  ...
  ${Tip} 锐速/LotServer/BBR ?恢С?OpenVZ！
